...
| Member photo | Image                  | Optional per member. If missing or the image fails to load, a placeholder avatar is used (see Edge Case 3).    | Bootstrap card-img-top, fixed aspect ratio, rounded corners, object-fit: cover so photos don't distort.                               |
